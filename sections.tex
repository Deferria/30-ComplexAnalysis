...
\section{复积分理论}
    ...
    \subsection{环绕数与Cauchy 积分公式}
        ...
        \begin{prf}
            ...
            所以该导数等于0, 并且只在有限个点中没有定义, 即 \(\exp h(t)=0\) 时. 显然该函数是连续的, 所以这是一个\hyperref[ppt:TrivialAnalyticalFunction]{常解析函数}. 对其应用初值, 即代入 \(t=\alpha\), 可得
    ...
    \subsection{高阶导数与解析函数的性质}
        ...
        \begin{thm}
            ...
            [Liouville's Theorem]
            [Dedicatia]
            在整个平面内都有界的\解析 必定是\hyperref[ppt:TrivialAnalyticalFunction]{常解析函数}. 
        \end{thm}
    ...
    \subsection{同调意义下的Cauchy定理}
        ...
        同时, \hyperref[dfn:ExactDifferential]{正合微分}的性质可以写为: 
        ...
        \begin{ppt}
            ...
            []
            [Dedicatia]
            如果一个微分 \(p\dd{x}+q\dd{y}\) 在 \(D\) 上的每个点的邻域都是\hyperref[dfn:ExactDifferential]{正合微分}, 那么对于 \(D\) 中任意的同调于零的闭链 \(\gamma\), 都有
    ...
    \begin{prf}
        我们回忆曾经的Cauchy 积分公式: 
        \[\Windn(\gamma,a)f(a)=\frac{1}{2\pi\ii}\int_\gamma\frac{f(z)}{z-a}\dd{z}\]
        要求 \(\gamma\) 在 \(D\) 上\hyperref[dfn:NullHomologous]{同调于零}且 \(f(z)\) 是\解析. 设 \(a_j\) 的去心邻域 \(0<|z-a_j|<\delta_j\) 上的闭圆环 \(C_j\) 半径小于 \(\delta_j\), 记
    ...
    \subsection{局部映射与幅角原理}
        ...
        \begin{crl}
            ...
            [Argument Principle]
            [Dedicatia]
            设 \(f(z)\) 是 \(D\) 上的\亚纯, 具有 \(m\) 个零点 \(a_j\) 和 \(n\) 个极点 \(b_k\),  \(\gamma\) 是 \(D\) 上的\hyperref[dfn:NullHomologous]{同调于零}的不过 \(a_j\) 与 \(b_k\) 的闭链, 有
        ...
        \begin{crl}
            ...
            []
            [Dedicatia]
            设 \(f(z)\) 是 \(D\) 上的\亚纯, 具有 \(m\) 个零点 \(a_j\) 和 \(n\) 个极点 \(b_k\),  \(\gamma\) 是 \(D\) 上的\hyperref[dfn:NullHomologous]{同调于零}的不过 \(a_j\) 与 \(b_k\) 的闭链,  \(g(z)\) 是定义在 \(D\) 上的\解析, 有
        ...
        \begin{thm}
            ...
            [Rouch\'e Theorem]
            [Dedicatia]
            设 \(f(z)\) 与 \(g(z)\) 在区域 \(D\) 上解析,  \(\gamma\) 是 \(D\) 上的\hyperref[dfn:NullHomologous]{同调于零}的闭链, 如果在 \(\gamma\) 上有
            \[|g(z)|<|f(z)|\]
...
\section{函数的级数展开与因子分解}
    ...
    \subsection{Weierstrass定理}
        ...
        \begin{thm}
            ...
            [Dedicatia]
            设 \(f_n\) 是定义在一簇开集 \(D_n\) 上的\解析, 函数列 \(\{f_n\}\) 在 \(D\) 上收敛于函数 \(f(z)\), 且在 \(D\) 的任意紧致子集上\UniformConvergent{} 于函数 \(f(z)\), 那么 \(f(z)\) 是解析函数, 并且 \(f'_n(z)\) 在 \(D\) 的任一紧致子集上都一致收敛于 \(f'(z)\).
        \end{thm}
        ...
        \begin{thm}
            ...
            设级数 \(\sum_{n = 1}^{\infty} u_n(z)\) 的每一项都是\解析, 且在 \(D\) 的任意紧致子集 \(D'\) 上\UniformConvergent{}, 那么级数的和函数 \(S(z)\) 在 \(D\) 上是\解析, 且可以逐项微分. 
    ...
    \subsection{亚纯函数的部分分式展开}
        ...
        我们从\hyperref[thm:Laurent-Series]{Laurent级数}中可以看到: 如果 \(f(z)\) 是一个\亚纯, 对极点 \(b_n\), 就会有 \(f(z)\) 的非解析部分 \(P_n\qty(\frac{1}{z-b_n})\). 如此减去所有的非解析部分之后就会得到解析的函数 \(g(z)\). 也就是说我们可以将 \(f(z)\) 分解为
        ...
        \begin{dfn}
            ...
            整函数有以下三类: 
            \begin{enumerate}
                \item \hyperref[ppt:TrivialAnalyticalFunction]{常解析函数}; 
                \item 有理函数, 即多项式; 
        ...
        \begin{ppt}
            ...
            上述三类整函数有如下的性质:
            \begin{enumerate}
                \item 整函数是常解析函数当且仅当无穷远点是它的\hyperref[dfn:RemovableSingularity]{可去奇点};
